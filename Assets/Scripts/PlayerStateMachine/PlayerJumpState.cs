...
public class PlayerJumpState : PlayerBaseState, IRootState
{
    ...
    void HandleJump()
    {
        if (Ctx._jumpCount < 3 && Ctx.CurrentJumpResetRoutine != null)
        {
    ...
    public void HandleGravity()
    {
        ...
        if(isFalling)
        {
            ...
            Ctx.AppliedMovementY = Mathf.Max((Ctx.CurrentMovementY + previousYVelocity) * .5f, -20f);
        }
        else
        {
            float previousYVelocity = Ctx.CurrentMovementY;
            Ctx.CurrentMovementY = Ctx.CurrentMovementY + (Ctx.JumpGravities[Ctx.JumpCount] * Time.deltaTime);
            Ctx.AppliedMovementY = (Ctx.CurrentMovementY + previousYVelocity) * .5f;
        }
    }
